...
> **Critical: Each service must run in its own terminal/console window**
>
...
> **Important**: Environment variables are automatically generated when you run `azd provision`. You may need to manually configure some values as described in the steps below.
...
> **Important**: The `azd provision` command can take 15-30 minutes to complete. Do not interrupt this process.
...
> **Important**: Make sure this port matches the proxy configuration in `vite.config.ts` (Step 9.3)
...
> **Hot Reload**: Changes to React components will automatically reload in the browser. No need to manually refresh.
...
> **Important**: Replace `<env-name>` with your actual azd environment name. You can find it by running `azd env list`.
...
> **Important**: You may need to stop the deployed function in the Azure Portal to ensure all requests are processed locally during debugging.
