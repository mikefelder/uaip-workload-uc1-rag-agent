...
Run without parameters to check default models and regions without verbose logging:
   ```
...
Enable verbose logging:
  ...
  ./quota_check_params.sh --verbose
   ```
Check specific model(s) in default regions:
  ```
  ./quota_check_params.sh --models gpt4.1:150,text-embedding-ada-002:100
  ```
Check default models in specific region(s):
```
./quota_check_params.sh --regions eastus2,japaneast
```
Passing both models and regions:
```
